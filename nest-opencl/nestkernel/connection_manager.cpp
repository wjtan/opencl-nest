  ...
  if ( sgid < connections_[ t ].size() )
  {
    ...
    if ( p != 0 ) // only send, if connections exist
    {
      if ( has_secondary( p ) )
      {
        ...
        {
	  //printf("built_connections 1: %d\n", hh_psc_alpha_gap::built_connections);
	  //if (not kernel().simulation_manager.gpu_execution[t]->built_connections)
	    p->send_secondary(
			      e, t, kernel().model_manager.get_synapse_prototypes( t ) );
	    //kernel().simulation_manager.gpu_execution[t]->fill_event_buffer(e);
        }
      }
    }
  }
